...
PortAudioDriver::PortAudioDriver(Master *useMaster) : AudioDriver("PortAudio"), master(useMaster) {
	start();
}
...
PortAudioDevice::PortAudioDevice(PortAudioDriver *driver, int useDeviceIndex, QString useDeviceName) : AudioDevice(driver, useDeviceName), deviceIndex(useDeviceIndex) {
}
